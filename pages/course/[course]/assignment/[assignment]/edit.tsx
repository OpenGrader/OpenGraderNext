...
export const getServerSideProps = async (ctx: GetServerSidePropsContext) =>
  withProtected(ctx, async (ctx) => {
    ...
    const { data: assignment } = await supabase
      .from("assignment")
      .select("id, title, description, is_open, is_late, input_file, output_file, language, args")
      .eq("id", ctx.query?.assignment)
      .single();
...
const EditAssignment: NextPage<EditAssignmentProps> = (props) => {
  ...
  const [lang, setLang] = useState<string>(props.language ?? "c/c++");
  ...
  return (
    <div className="flex">
      <div className="text-gray-100 px-12 pt-6 flex flex-col gap-4 w-full">
        <h1 className="font-bold text-3xl">Update assignment: {props.title}</h1>
